...
function Get-LatestRelease {
    try {
		...
		$releases = Invoke-RestMethod -Uri 'https://api.github.com/repos/bdwyzr/winutil/releases'
...
function RedirectToLatestPreRelease {
    ...
    if ($latestRelease) {
		...
		$url = "https://github.com/bdwyzr/winutil/releases/download/$latestRelease/winutil.ps1"
    } else {
		...
		$url = "https://github.com/bdwyzr/winutil/releases/latest/download/winutil.ps1"
    }
